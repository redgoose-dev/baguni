...
async function uploadFile(file)
{
  ...
  try
  {
    ...
    if (res?.data?.id)
    {
      ...
      state.queue[res.data.id] = res.data.file
      toast.add('파일추가 성공', 'success').then()
    }
    else
    {
      toast.add('파일추가 실패', 'error').then()
    }
  }
  catch
  {
    toast.add('파일추가 오류', 'error').then()
  }
}
async function uploadFiles(files)
{
  ...
  {
    Object.entries(state.queue).forEach(([ key, value ]) => {
...
function onActionItem(action, id)
{
  ...
  {
    case 'new-window':
      window.open(`/file/${id}/?_a=${auth.token}`)
      break
  ...
  switch (type)
  {
    case 'markdown':
      code = createMarkdownItems(ids.map(id => {
        ...
        return {
          name: item.name,
          path: `${assetContentBody.host}/file/${item.id}/`,
          type: item.type,
        }
      }))
      break
