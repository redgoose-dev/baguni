...
const $index = computed(() => {
  return data.index.map(o => {
    return {
      ...
      thumbnail: o.cover_file_id ? `/file/${o.cover_file_id}/?_a=${auth.token}` : null,
...
async function fetch()
{
  try
  {
    ...
    let res = await request('/collection/', {
      method: 'get',
    })
...
async function onSubmit()
{
  try
  {
    ...
    await request(`/asset/${props.assetId}/collection/`, {
      method: 'patch',
      body: formData({ collections: ids.value.join(',') }),
    })
