  ...
  <form @submit.prevent>
    ...
    <div class="field">
      ...
      <dl class="date-range">
        ...
        <dd>
          <InputText
            type="date"
            v-model="asset.filter.dateStart"
            id="dateStart"
    ...
    <div class="field">
      ...
      <div>
        <RadioButton
          name="theme"
          v-model="asset.filter.indexTheme"
          size="small"
          :only-icon="false"
          :options="[
          ...
          @update:modelValue="asset.saveFilter()"/>
      </div>
    </div>
    <div class="field">
      <span class="label">태그</span>
      <div class="tags">
        <ul v-if="asset.filter.tags?.length > 0">
          <li v-for="tag in asset.filter.tags">
            <Tag
              :label="tag.name"
              :use-remove="true"
              :fill="false"
...
{
  const idx = findObjectByKey(asset.filter.tags, 'id', id)
  asset.filter.tags.splice(idx, 1)
  asset.saveFilter()
  emits('update')
}
