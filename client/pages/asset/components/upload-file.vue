  ...
  <div
    ...
    <ShadowBox class="upload-file__box">
      <figure class="upload-file__body">
        ...
        <figcaption>
          <h4>에셋파일 업로드</h4>
          <p>최대 {{$limitUploadMainFileSize}} 사이즈의 파일까지 업로드</p>
        </figcaption>
      </figure>
...
const $preview = computed(() => {
  if (typeof props.file === 'number')
  {
    return {
      ...
      src: `/file/${props.file}/?_a=${auth.token}`,
...
const $meta = computed(() => {
  ...
  if (meta.date) result.date = dateFormat(meta.date, '{yyyy}-{MM}-{dd}')
  if (meta.image) result.imageSize = `${meta.image.width}px*${meta.image.height}px`
  return result
})
const $limitUploadMainFileSize = computed(() => {
  return getByte(auth.preference?.file.limit.main)
})
  ...
  if (value)
  {
    ...
    switch (type)
    {
      ...
      case 'meta':
        ...
        {
          meta.image = await getImageSize(`/file/${props.file}/?_a=${auth.token}`)
        }
        break
    }
  }
...
{
  const limitSize = auth.preference?.file.limit.main
