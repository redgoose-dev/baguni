...
<teleport to="#modal">
  <Modal
    ...
    <AttachmentFiles
      ...
      :limit="auth?.preference?.file?.limit?.bodyCount"
...
const forms = reactive({
  ...
  file: props.data?.file ? {
    ...
    date: new Date(props.data.file.created_at),
  } : {},
  cover: pureObject(props.data?.json?.cover) || null,
})
const files = reactive({
  main: props.data?.file?.id || undefined,
  coverOrigin: props.data?.cover_origin || undefined,
  coverCreate: props.data?.cover_create || undefined,
  removedMain: !Boolean(props.data?.file?.id),
  removedCover: !Boolean(props.data?.cover_origin),
})
...
const $submitLabel = computed(() => {
  ...
  return $isEdit.value ? '수정하기' : '에셋 만들기'
})
...
{
  const { coordinates, origin, create, removedCover } = src
  if (removedCover)
  {
    forms.cover.coordinates = null
    files.coverOrigin = null
  ...
  if (origin) files.coverOrigin = origin
...
async function onSubmit()
{
  ...
  if (files.coverOrigin && files.coverOrigin instanceof File) body.cover_origin = files.coverOrigin
  if (files.coverCreate && files.coverCreate instanceof File) body.cover_create = files.coverCreate
  body.remove_files = [
    files.removedMain && 'main',
    files.removedCover && 'cover-origin',
