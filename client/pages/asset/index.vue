    ...
    <div :class="[
      ...
      <div class="explorer__content">
        ...
        <ul
          ...
          :class="[
            'explorer__index',
            asset.filter.indexTheme === 'list' && 'list',
            asset.filter.indexTheme === 'thumbnail' && 'thumbnail',
          ]">
        ...
        <nav class="explorer__paginate">
          <Paginate
            ...
            :total="data.total"
            :size="asset.indexSize"
            :range="asset.paginateSize"
...
const $index = computed(() => {
  ...
  return data.index.map(item => {
    const { id, title, description, cover_file_id, file_id, type, created_at } = item
...
async function fetch()
{
  try
  {
    ...
    let query = {
      ...
      size: asset.indexSize,
      order: asset.filter.order || undefined,
      sort: asset.filter.sort || undefined,
      date_start: asset.filter.dateStart,
      date_end: asset.filter.dateEnd,
      file_type: asset.filter.fileType,
      tags: asset.filter.tags?.length > 0 ? asset.filter.tags.map(o => (o.id)).join(',') : undefined,
      q: queryParams.q,
    }
    // filtering query
...
{
  asset.filter.open = !asset.filter.open
  asset.saveFilter()
}
</script>
